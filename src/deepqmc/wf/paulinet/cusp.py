...
class CuspCorrection(nn.Module):
  ...
  def _fit_cusp_poly(self, phi_gto_boundary, mos0):
    ...
    phi_gto_boundary, mos0, shifts, charges, rc = (x[..., has_s_part]
                                                   for x in (phi_gto_boundary,
                                                             mos0, self.shifts,
                                                             charges, rc))
  ...
  def forward(self, rs_2, phi_gto_boundary, mos0):
    ...
    params_idx[has_s_part] = torch.arange(has_s_part.sum(),
                                          device=params_idx.device)
    rs_1_idx = torch.arange(len(rs_1)).cuda()[:, None].expand(
        -1, mos0.shape[-1])[corrected[maybe_corrected]]
    C, sgn, *alphas = torch.stack([C, sgn,
                                   *alphas])[:,
